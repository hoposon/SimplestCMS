...
CREATE TABLE app_public.USERS_URLS (
	...
	url_id INT REFERENCES app_public.URLS(id)
);
...
CREATE TABLE app_public.CONTENT_STRINGS (
	id SERIAL PRIMARY KEY,
	page_id INT REFERENCES app_public.PAGES(id),
	cs_name VARCHAR(200),
	cs_code VARCHAR(200),
	cs_value TEXT,
	created_at timestamptz NOT NULL DEFAULT now(),
...
CREATE TABLE app_public.PAGE_CONTENT_STRINGS (
	id SERIAL PRIMARY KEY,
	cs_id INT REFERENCES app_public.CONTENT_STRINGS(id),
	page_id INT REFERENCES app_public.PAGES(id),
	created_at timestamptz NOT NULL DEFAULT now(),
	updated_at timestamptz NOT NULL DEFAULT now(),
	published BOOLEAN
...
CREATE TABLE app_public.ASSETS (
	...
	directory INT REFERENCES app_public.DIRS(id),
	created_at timestamptz NOT NULL DEFAULT now(),
  	updated_at timestamptz NOT NULL DEFAULT now()
);
...
CREATE TABLE app_public.PAGE_ASSETS (
	id SERIAL PRIMARY KEY,
	asset_id INT REFERENCES app_public.ASSETS(id),
	page_id INT REFERENCES app_public.PAGES(id),
...
CREATE TABLE app_public.PAGE_GALLERY (
	id SERIAL PRIMARY KEY,
	gallery_id INT REFERENCES app_public.GALLERY(id),
	page_id INT REFERENCES app_public.PAGES(id),
	created_at timestamptz NOT NULL DEFAULT now(),
  	updated_at timestamptz NOT NULL DEFAULT now(),
	published BOOLEAN
